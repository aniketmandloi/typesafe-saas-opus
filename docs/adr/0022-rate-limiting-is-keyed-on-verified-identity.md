...
**The bucket table is an ordinary logged Drizzle table under [ADR-0012](./0012-migrations-are-a-deploy-step-under-their-own-profile.md)'s migration path**, and `UNLOGGED` is rejected rather than overlooked ([#45](https://github.com/aniketmandloi/typesafe-saas-opus/issues/45)).

On Neon the saving is real — `pagestore_smgr.c` dispatches on `smgr_relpersistence` and routes `RELPERSISTENCE_UNLOGGED` to the stock local file manager, so nothing reaches the safekeepers or the metered restore history. On Aurora it is illusory, and AWS says so in its own hedged wording: unlogged tables there are crash-safe *because* "the Aurora storage system does not rely on the traditional PostgreSQL WAL for durability", and consequently "the performance benefits typically associated with unlogged tables in standard PostgreSQL may not be as significant in Aurora." Durability and skipping the write path cannot both be true. **One schema has to hold on both reference targets, so the table stays logged** — the saving exists on one of them and nothing measurable on the other.

Two costs would have ridden along regardless. Drizzle 0.45 cannot express `UNLOGGED` at all ([drizzle-orm#5347](https://github.com/drizzle-team/drizzle-orm/issues/5347) is open), so it would have to live in a hand-edited migration, invisible to the typed schema and to any `drizzle-kit` diff — the one table property outside ADR-0012's path. And Neon truncates unlogged relations on *any* compute restart, not merely an unclean one; the reset leaves the relation valid but empty, so the upsert finds no conflict, takes the INSERT arm and **fails open, silently**, for one window. Harmless for a disposable counter, invisible when it happens.

What would reopen this is a measurement showing the bucket table's WAL is a material share of Neon's billed `instant_restore_bytes_month` — the Neon saving is established and metered, just unsized. The honest shape then is a Neon-profile-specific migration, not a schema flag. **The lever worth measuring first is `fillfactor` plus per-table autovacuum**: the write path is an UPSERT hammering the same rows, `count` and its timestamp are unindexed, and that is textbook HOT-update shape — portable across all three platforms with no truncation semantics attached.
