// The identity tables. Bootstrapped once from `@better-auth/cli`, then owned
// here (ADR-0011): `auth generate` overwrites this whole file and the CLI that
// runs it is two minors behind the library, so it is never re-run. Changes are
// applied by hand and proved by the checkSchema() gate in auth.test.ts.
//
// A kit column added to one of these tables must be nullable or carry a
// default. Better Auth inserts without it, and the gate rejects the schema
// otherwise.
...
export const organization = pgTable(
  ...
  {
    id: text("id").primaryKey(),
    // ADR-0007: the only marker of a Dark Organization. Nullable, so Better
    // Auth's own inserts are unaffected.
